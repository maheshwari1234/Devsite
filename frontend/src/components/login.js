...
store=(key,value)=>{
    ...
    axios.post("http://localhost:2000/login",{email,password}).then((res)=>{
        this.setState({submitted:true})
                    this.setState({successMessage:"Successfully Logged in",logged:true})
                    this.props.history.push({pathname:"/"})
                })
                .catch(err=>{
                    this.setState({errorMessage:"Logged in failed"})
                })
